...
c[8];if(c[9]&&(b.sec=c[10],c[11]))b.milisec=Number("0"+c[11])*1E3;if(c[12]&&(b.type=3,b.offset=0,c[14]&&(b.offset=Number(c[14])*60+Number(c[15]),c[13]=="-")))b.offset=-b.offset}return b}}(),g={color:{restrictions:{type:function(a,c){var b=a.val();return b==""&&c||/^#[0-9A-F]{6}$/i.test(b)}}},date:{restrictions:{type:function(a,c){var b=a.val();return b==""&&c||p(b)},min:function(a){var c=i(a.attr("min")),a=i(a.val());return isNaN(a)||c<=a},max:function(a){var c=i(a.attr("max")),a=i(a.val());return isNaN(a)||
a<=c}}},datetime:{restrictions:{type:function(a,c){var b=a.val();return b==""&&c||b.match(/^(\d{4}-\d{2}-\d{2})T(.+)(Z|(\+|-)([01][0-9]|2[0-3]):([0-5][0-9]))$/)&&function(){var a=RegExp.$2;return p(RegExp.$1)&&q(a)}()},min:function(a){return g.date.restrictions.min(a)},max:function(a){return g.date.restrictions.max(a)}}},"datetime-local":{restrictions:{type:function(a,c){var b=a.val();return b==""&&c||b.match(/^(\d{4}-\d{2}-\d{2})T(.+)$/)&&function(){var a=RegExp.$2;return p(RegExp.$1)&&q(a)}()},
...
!l.hasClass(h)||!e.callbackOnlyErrorTransitions){l.addClass(h);try{G(l)}catch(a){throw"ht5ifv: error in callback ("+a+")";}}}).addClass(u);b(l,f);a(l,f);l.bind(f.events.validate,function(){d(this).trigger("validate.ht5ifv")}).bind(f.events.check,function(){d(this).trigger("check.ht5ifv")})}})}function f(){q.each(function(){var a=d(this),b=[];d.each(d("option[selected]",a),function(){var a=d(this),a=a.attr("value")||a.text();b.push(a)});a.val(b)});j.each(function(){var a=d(this),b=[];j.filter('[name="'+
...
q.each(function(){c(d(this),e.select)});var s=function(){return g.is("form")?g:function(){var a=g.parents("form").first();return a.is("form")?a:d("body")}()}();k.add(j).each(function(){var c=d(this),f=e[c.attr("type")];d.each(f.restrictions,function(g,h){if(c.is("["+g+"]")){var i=t[g]="ht5ifv-"+g+"-error",j=f.targets[g](c),o=f.callbacks[g],m=f.classes[g],k=d('input[type="'+c.attr("type")+'"][name="'+c.attr("name")+'"]',s);k.filter("["+g+"]").first().is(c)&&(k.bind("validate.ht5ifv",function(){var a=
d(this);if(!e.checkDisable||!a.parents("form,fieldset").andSelf().is("[disabled]"))if(a.hasClass("ht5ifv-radioControlGroup")||(k.addClass("ht5ifv-radioControlGroup"),k.not(a.get(0)).trigger("validate.ht5ifv")),a.removeClass("ht5ifv-radioControlGroup"),h(k,a,e.ignoreEmptyFields))j.removeClass(m),a.removeClass(i);else if(j.addClass(m),!a.hasClass(i)||!e.callbackOnlyErrorTransitions){a.addClass(i);try{o(a)}catch(b){throw"ht5ifv: error in callback ("+b+")";}}}).addClass(u),b(k,f),a(k,f),k.bind(f.events.validate,
function(){d(this).trigger("validate.ht5ifv")}).bind(f.events.check,function(){d(this).trigger("check.ht5ifv")}))}})});var o=d.extend({},t,{valid:"ht5ifv-valid-status",invalid:"ht5ifv-invalid-status"}),m=h.filter("."+u);e.browserValidate!=!0&&s.attr("novalidate","novalidate");d("input[type='reset']",s).click(function(){m.trigger("clear.ht5ifv")});e.validateOnSubmit&&s.bind("submit.ht5ifv",function(){return r()});var r=function(){m.trigger("validate.ht5ifv").trigger("check.ht5ifv");var a=!0;m.each(function(){a=
a&&!d(this).hasClass(o.invalid)});return a};g.data("ht5ifv",{form:g,method:{_validate:function(){m.trigger("validate.ht5ifv").trigger("check.ht5ifv")},_check:r,_reset:function(){m.trigger("clear.ht5ifv");try{s.get(0).reset()}catch(a){console.warn('The DOM reset method does not exist. It was problably overriden. Chech in your form for a tag with the value "reset" assigned to an attribute id or name. It just a hint. I will try again in a different way');var b=d('[type="reset"]',s);b.length?b.first().click():
(console.warn("Reset form: the last try. Fallback to reset_fields only"),f())}},_defaults:function(){m.trigger("clear.ht5ifv");f()},_clean:function(){m.trigger("clear.ht5ifv");i.val("");p.val("");j.val(["__ht5ifv__some_unprobably_text_algum_texto_"]);k.val(["__ht5ifv__some_unprobably_text_algum_texto_"]);q.val(["__ht5ifv__some_unprobably_text_algum_texto_"])},_destroy:function(){m.trigger("clear.ht5ifv");m.unbind(".ht5ifv");g.unbind(".ht5ifv");g.removeData("ht5ifv")}}})}})},validate:function(){return this.each(function(){try{d(this).data("ht5ifv").method._validate()}catch(a){throw console.error(a),
console.log("The object Error is: %o",a),'ht5ifv: "validate" error ('+a+")";}})},valid:function(){var a=!0;this.each(function(){try{a=a&&d(this).data("ht5ifv").method._check()}catch(c){throw'ht5ifv: "valid" error ('+c+")";}});return a},reset:function(){return this.each(function(){try{d(this).data("ht5ifv"),d(this).data("ht5ifv").method._reset()}catch(a){throw'ht5ifv: "reset" error ('+a+")";}})},reload:function(){return this.each(function(){try{d(this).data("ht5ifv"),d(this).data("ht5ifv").method._defaults()}catch(a){throw'ht5ifv: "default" error ('+
a+")";}})},clean:function(){return this.each(function(){try{d(this).data("ht5ifv").method._clean()}catch(a){throw'ht5ifv: "clean" error ('+a+")";}})},destroy:function(){return this.each(function(){try{d(this).data("ht5ifv").method._destroy()}catch(a){throw'ht5ifv: "destroy" error ('+a+")";}})}};d.fn.ht5ifv=function(a){if(x[a])return x[a].apply(this,Array.prototype.slice.call(arguments,1));else if(a instanceof Object||!a)return x.init.apply(this,arguments);else d.error("Method "+a+" does not exist on jQuery.ht5ifv plugin")};
var h={},k={extend:function(a,c,b){w.push({definitions:a,global:c,selfContained:b})},register:function(a,c,b){a&&(h[a]===void 0||b)?h[a]=c:console.warn("Cannot registry the handler with name: %s",a)},unregister:function(a){delete h[a]},clean:function(){h={}},registerType:function(a,c,b){k.register(a,function(){return{types:function(b){b[a]={restrictions:{type:c}};return b}({})}},b)},registerRestriction:function(a,c,b){k.register(a,function(){return{restrictions:function(b){b[a]=c;return b}({})}},
b)},use:function(a){if(a)for(var c=0;c<arguments.length;c++){var b=arguments[c];if(typeof b=="string")h[b]?d.ht5ifv("extend",h[b].apply(this)):console.warn("Cannot find the handler with name: %s",b);else if(b instanceof Array){var f=b.shift();f&&h[f]?d.ht5ifv("extend",h[f].apply(this,b)):console.warn("Cannot find the handler with name: %s",b)}}else d.each(h,function(a,b){d.ht5ifv("extend",b.apply(this))})}};d.ht5ifv=function(a){if(k[a])return k[a].apply(this,Array.prototype.slice.call(arguments,1));
else d.error("Static Method "+a+" does not exist on jQuery.ht5ifv plugin")}})(jQuery);